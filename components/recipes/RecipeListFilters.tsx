  ...
  return (
    <section className="space-y-2">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="flex w-full items-center justify-between rounded-xl bg-surface-container px-3 py-2.5 text-left text-sm font-medium"
        aria-expanded={open}
      >
        <span>
          {open ? "▲" : "▼"} 検索
      ...
      {open ? (
        <div className="space-y-3 rounded-2xl bg-surface-container-lowest p-4 ring-1 ring-outline-variant">
          <label className="block space-y-2">
            <span className="text-sm font-medium text-on-surface">検索ワード</span>
            <input
              type="search"
              value={filter.query}
              onChange={(event) =>
          ...
          <details className="rounded-xl bg-surface-container p-3">
            ...
            <div className="mt-3 grid grid-cols-2 gap-3">
              <label className="text-xs">
                作る人
                <select
                  value={filter.cookMemberId ?? ""}
                  onChange={(e) =>
                    onChange({ ...filter, cookMemberId: e.target.value })
                  }
                  className="mt-1 w-full rounded-lg bg-surface-container-lowest p-2 text-sm"
                >
                  <option value="">指定なし</option>
                  {cookingProfiles.map((profile) => (
                    <option
                      key={profile.id}
                      value={profile.familyMemberProfileId}
                    >
                      {profile.familyMemberProfileId}
                    </option>
                  ))}
                </select>
              </label>
              <label className="text-xs">
                ...
                <input
                  type="number"
                  value={filter.maxCookingMinutes ?? ""}
                  onChange={(e) =>
                    onChange({
                      ...filter,
                      maxCookingMinutes:
              ...
              {(
                [
                  ["lowCleanupOnly", "洗い物少なめ"],
                  ["noDeepFrying", "揚げ物なし"],
                  ["beginnerFriendlyOnly", "初心者向け"],
                  ["familiarOnly", "慣れた料理"],
                  ["learningOnly", "挑戦中"],
                  ["makeAheadOnly", "作り置き向き"],
                ] as const
              ).map(([key, label]) => (
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="flex-1 rounded-xl px-4 py-2.5 text-sm font-medium text-primary ring-1 ring-outline-variant"
            >
              閉じる
            </button>
          </div>
